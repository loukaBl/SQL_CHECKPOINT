...
--  Afficher les employés hommes avec salaire net >= 8000 par ancienneté décroissante
SELECT 
    EMPLOYEE_NUMBER,
    FIRST_NAME,
    LAST_NAME,
...
--  Afficher les produits répondant aux critères
SELECT 
...
--  Clients dans la même région que le fournisseur 1
SELECT *
FROM CUSTOMERS
...
--  Nouveau taux de remise pour les commandes entre 10998 et 11003
SELECT 
    ORDER_NUMBER,
    CASE 
        WHEN ORDER_NUMBER BETWEEN 10000 AND 10999 THEN NULL
...
--  Fournisseurs de produits de type beverage
SELECT DISTINCT 
    S.SUPPLIER_NUMBER,
    S.COMPANY,
    S.ADDRESS,
...
--  Clients de Berlin ayant commandé au maximum 1 dessert
...
WHERE C.CITY = 'Berlin' 
  AND (CAT.CATEGORY_NAME LIKE '%dessert%' OR CAT.CATEGORY_NAME IS NULL)
GROUP BY C.CUSTOMER_CODE
HAVING COUNT(DISTINCT P.PRODUCT_REF) <= 1;

--  Clients français et montant total chaque lundi d'avril 1998
SELECT 
    C.CUSTOMER_CODE AS CUSTOMER_NUMBER,
    C.COMPANY AS COMPANY_NAME,
    C.PHONE,
    ISNULL(SUM(OD.UNIT_PRICE * OD.QUANTITY * (1-OD.DISCOUNT/100)),0) AS TOTAL_AMOUNT,
    C.COUNTRY
FROM CUSTOMERS C
LEFT JOIN ORDERS O ON C.CUSTOMER_CODE = O.CUSTOMER_CODE
LEFT JOIN ORDER_DETAILS OD ON O.ORDER_NUMBER = OD.ORDER_NUMBER
...
--  Nombre de commandes par client français
SELECT 
    C.CUSTOMER_CODE,
...
--  Nombre de commandes en 1996 et 1997 et différence
SELECT 
    SUM(CASE WHEN YEAR(O.ORDER_DATE) = 1996 THEN 1 ELSE 0 END) AS ORDERS_1996,
    SUM(CASE WHEN YEAR(O.ORDER_DATE) = 1997 THEN 1 ELSE 0 END) AS ORDERS_1997,
    SUM(CASE WHEN YEAR(O.ORDER_DATE) = 1997 THEN 1 ELSE 0 END) - SUM(CASE WHEN YEAR(O.ORDER_DATE) = 1996 THEN 1 ELSE 0 END) AS DIFFERENCE
FROM ORDERS O;
